...
namespace robot::io
{
    ...
    public:
        ...
        size_t Send(const T *buf, size_t size)
        {
            try
            {
                return m_serial_port->write_some(boost::asio::buffer(buf, size));
            }
        ...
        size_t Receive(T *data, int count)
        {
            try
            {
                size_t len = m_serial_port->read_some(boost::asio::buffer(data, count));
                return len;
            }
            catch (std::exception &e)
            {
        ...
        {
            boost::asio::steady_timer timer(m_io_service);
            std::optional<boost::system::error_code> timerResult = std::nullopt;
            ...
            m_serial_port->async_read_some(
                boost::asio::buffer(data, count),
                [&readResult, &bytesReaded](const boost::system::error_code &error, size_t readed)
                {
                    readResult = error;
                    bytesReaded = readed;
                });
